...
namespace ResourceBox.Services.Data
{
    ...
    {
        private string connection = @"workstation id=resourceboxdb.mssql.somee.com;packet size=4096;user id=a3719456_SQLLogin_1;pwd=85h9jhw494;data source=resourceboxdb.mssql.somee.com;persist security info=False;initial catalog=resourceboxdb ";
        ...
        public void updateProfileInfo(User user)
        {
            using (SqlConnection sqlcon = new SqlConnection(connection))
            {
                string sqlquery = "UPDATE Users SET address='" + user.address + "', university='" + user.university + "', userName='" + user.userName + "', phone='" + user.phone + "', year='" + user.year + "', semester='" + user.semester + "' WHERE userId=" + user.userId;
                using (SqlCommand sqlcom = new SqlCommand(sqlquery, sqlcon))
                {
                    sqlcon.Open();
                    sqlcom.ExecuteNonQuery();
        ...
        public void updateProfilePicture(String path,int userId)
        {
            using (SqlConnection sqlcon = new SqlConnection(connection))
            {
                string sqlquery = "UPDATE Users SET userImage='" + path + "' WHERE userId=" + userId;
                using (SqlCommand sqlcom = new SqlCommand(sqlquery, sqlcon))
                {
                    sqlcon.Open();
                    sqlcom.ExecuteNonQuery();
        ...
        public void createUserNotification()
        {
            ...
            using (SqlConnection sqlcon = new SqlConnection(connection))
            {
                string sqlquery2 = "SELECT TOP 1 userId FROM Users ORDER BY userId DESC";
                using (SqlCommand sqlcom = new SqlCommand(sqlquery2, sqlcon))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        lastUserID = Convert.ToInt32(reader["userId"]);
            ...
            using (SqlConnection sqlcon = new SqlConnection(connection))
            {
                string sqlquery = "insert into UserNotifications(userId,messageUnread,newNotifications) values ('" + lastUserID + "','" + value + "','" + value + "')";
            using (SqlCommand sqlcom = new SqlCommand(sqlquery, sqlcon))
            {
                sqlcon.Open();
                sqlcom.ExecuteNonQuery();
        ...
        public List<User> fetchUser()
        {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                while (rdr.Read())
                {
                    ...
                    user.university = rdr["university"].ToString();
                    user.year = Convert.ToInt32(rdr["year"]);
                    user.semester = Convert.ToInt32(rdr["semester"]);
                    userList.Add(user);
                }
            }
            return userList;
        ...
        public List<UserChat> fetchMessages()
        {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                rdr.Close();
                cmd = new SqlCommand("SELECT USERNAME1.messageId, SenderId, Sender,SenderImage, RecieverId, Receiver,RecieverImage, USERNAME1.message,date_time ,seen FROM USERNAME1 INNER JOIN USERNAME2 ON USERNAME1.messageId = USERNAME2.messageId ORDER BY messageId ASC", con);
                cmd.CommandType = CommandType.Text;
